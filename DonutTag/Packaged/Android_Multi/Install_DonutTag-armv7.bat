...
%ADB% %DEVICE% install DonutTag-armv7.apk
...
%ADB% %DEVICE% shell rm -r %STORAGE%/Download/obb/com.YourCompany.DonutTag
@echo.
@echo Installing new data. Failures here indicate storage problems (missing SD card or bad permissions) and are fatal.
%ADB% %DEVICE% push main.1.com.YourCompany.DonutTag.obb %STORAGE%/obb/com.YourCompany.DonutTag/main.1.com.YourCompany.DonutTag.obb
if "%ERRORLEVEL%" NEQ "0" goto Error
...
%ADB% %DEVICE% shell pm grant com.YourCompany.DonutTag android.permission.READ_EXTERNAL_STORAGE
%ADB% %DEVICE% shell pm grant com.YourCompany.DonutTag android.permission.WRITE_EXTERNAL_STORAGE
